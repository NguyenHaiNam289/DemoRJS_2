...
function App() {
  ...
  const getInfoUser = () => {
    sendHttpRequest("GET", urlInfo, "", accessToken)
      .then((responseData) => Gui_info(responseData.data))
      .catch((responseData) => infoError(responseData));
  };

  const getHocPhan = () => {
    sendHttpRequest("GET", urlHP).then((responseData) =>
      Gui_HocPhan(responseData.data)
    );
  };
  ...
  function infoError(err) {
    console.log(err);
    document.getElementById("Info").innerHTML =
      "Bạn chưa Đăng nhập hoặc đã Đăng Nhập quá lâu";
  }
  ...
  function Gui_HocPhan(data) {
    // const dsLopHocPhan = Object.entries(data.dsLopHocPhan);
    const dsLHP = data.dsLopHocPhan;
    var html = `
        <table>
            <tr><td colSpan="4" border= "1px"><h4>Danh sách lớp học phần</h4></td></tr>
    `;
    for (let i = 0; i < dsLHP.length; i++) {
      html += `
        <tr>
            <th>Mã lớp học phần</th>
            <td>${dsLHP[i].maLopHocPhan}</td>
            <th>Tên lớp học phần</th>
            <td>${dsLHP[i].tenLopHocPhan}</td>
        </tr>
        <tr>
            <th>Số lượng sv tối đa</th>
            <td>${dsLHP[i].soLuongSVDangKyToiDa}</td>
            <th>Số lương sv hiện tại</th>
            <td>${dsLHP[i].soLuongSVDangKyHienTai}</td>
        </tr>
        <tr>
            <th>Trạng thái</th>
            <td>${dsLHP[i].trangThai}</td>
            <td></td>
            <td></td>
        </tr>
      `;
    ...
    html += `</table>`;
    document.getElementById("Gui_HP").innerHTML = html;
  }
